...
/* ═══════════════════════════════════════════════════════════
   Director Dependency View — Enhanced with Approval Actions
   ═══════════════════════════════════════════════════════════ */
function DirectorDependencyView() {
  const { currentUserId, currentRole } = useWorkflowStore()
  ...
  const handleApproval = (step: any, action: string) => {
    approvalMutation.mutate({
      ...
      action,
      comments: actionComments || `${action} by ${currentUserId}`,
      ...
      <div key={step.id} style={{ border: `1px solid ${colors.border}30`, borderRadius: 8, padding: 14, marginBottom: 10, background: colors.bg, borderLeft: `3px solid ${colors.border}` }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <button
            ...
            onClick={() => handleApproval(step, 'APPROVE')}
            disabled={approvalMutation.isPending}
          >
            {stage === 'Director' ? '✓ Approve & Send to EA' : stage === 'EA Final' ? '✓ Final Submit' : '✓ Approve'}
          </button>
          {stage === 'Director' && (
            <button
              className="btn btn-xs"
              style={{ background: 'var(--amber-l)', color: 'var(--amber)', border: '1px solid var(--amber)' }}
              onClick={() => handleApproval(step, 'SEND_BACK')}
              disabled={approvalMutation.isPending}
            >
              ↩ Send Back to EA
            </button>
          ...
          <button
            ...
            onClick={() => handleApproval(step, 'REJECT')}
            disabled={approvalMutation.isPending}
          >
            ✕ Reject
          </button>
  ...
  return (
    <div>
      ...
      <div className="lcard" style={{ padding: '14px 18px', marginBottom: 14, background: 'linear-gradient(135deg,rgba(109,40,217,.06),var(--card))', borderLeft: '3px solid var(--purple)' }}>
        <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1, color: 'var(--purple)', marginBottom: 10 }}>⚙ Approval Flow: Employee → EA → Director → EA Final → Complete</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
          ...
          <span style={{ color: 'var(--purple)', fontSize: 12, fontWeight: 900 }}>→</span>
          <div style={{ background: 'var(--amber-l)', color: 'var(--amber)', padding: '5px 10px', borderRadius: 7, fontSize: 10, fontWeight: 800 }}>📋 EA Final Review</div>
          <span style={{ color: 'var(--purple)', fontSize: 12, fontWeight: 900 }}>→</span>
      ...
      <div className="stat-grid sg-4" style={{ marginBottom: 14 }}>
        ...
        <div className="sc lux-border">
          ...
          <div className="sc-top">
            <div><div className="sc-label">Director Approval Pending</div><div className="sc-val" style={{ color: 'var(--purple)' }}>{directorSteps.length}</div></div>
      ...
      <div className="g2" style={{ marginBottom: 14 }}>
        ...
        <div className="lcard">
          <div className="ch">
            <div className="ct" style={{ color: 'var(--amber)' }}>📋 EA Review Stage</div>
        ...
        <div className="lcard" style={{ borderLeft: '3px solid var(--purple)' }}>
          <div className="ch">
            <div className="ct" style={{ color: 'var(--purple)' }}>👔 Director Approval Stage</div>
      ...
      {/* EA Final Review Stage */}
      <div className="lcard" style={{ marginBottom: 14, borderLeft: '3px solid var(--green)' }}>
        <div className="ch">
          <div className="ct" style={{ color: 'var(--green)' }}>✅ EA Final Review & Submit</div>
